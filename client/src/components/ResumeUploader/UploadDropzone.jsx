...
export default function UploadDropzone({ 
  ...
  return (
    <div 
      className={`relative overflow-hidden border-2 border-dashed rounded-2xl p-6 sm:p-12 text-center transition-all duration-300 ease-in-out backdrop-blur-md cursor-pointer
        ${dragActive 
        ...
        ${file ? 'border-solid border-white/20 cursor-default p-4 sm:p-8' : ''}`}
      onDragEnter={onDrag}
      onDragLeave={onDrag}
      onDragOver={onDrag}
      onDrop={onDrop}
    ...
    >
      ...
      {!file ? (
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-5 text-left w-full">
          ...
          <button 
            type="button" 
            className="bg-white/5 border border-white/10 hover:bg-white/10 hover:border-white/20 text-slate-100 px-4 py-2 rounded-lg text-xs font-medium cursor-pointer transition-all duration-200 w-full sm:w-auto text-center" 
            onClick={(e) => {
              e.stopPropagation();
              setFile(null);
            }}
